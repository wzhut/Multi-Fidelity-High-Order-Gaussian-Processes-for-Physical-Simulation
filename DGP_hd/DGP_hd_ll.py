...
class DGP_hd:
    ...
    def sample(self, flag):
        ...
        cov = tf.matmul(tf.transpose(B_i), tf.matmul(kBB, B_i)) + tf.exp(-log_tau_i) * tf.eye(self.out_d, dtype=util.tf_type)
        ...
        cov = tf.reshape(tf.diag_part(cov), (1,-1))
        Y_ll = -0.5 * np.log(2 * np.pi) - 0.5 * tf.reduce_sum(tf.log(cov)) / self.out_d  - 0.5 * tf.reduce_mean(tf.square(self.Y_test) / cov)
        return Y_pre, Y_ll
